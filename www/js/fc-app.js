...
window.FCApp = (function () {
  ...
  let _legacyFinancialStorageCleaned = false;
  ...
  function _renderSpendingChart() {
    ...
    if (!periodTxns.length) {
      svgEl.innerHTML = '<text x="50%" y="44" text-anchor="middle" fill="var(--fc-text-faint)" font-size="12" font-family="-apple-system,sans-serif">No spending data for this period</text>';
    ...
    const bars = buckets.map((b, i) => {
      ...
      if (b.total <= 0) return `<rect x="${x}" y="${y}" width="${barW}" height="${h}" rx="${rx}" fill="var(--fc-border)"/>`;
      if (b.isNow) {
  ...
  function _generateSmartInsights() {
    ...
    if (periodIncome > 0 && periodSpend > 0) {
      ...
      if (savingsRate < 0) {
        ...
        if (savingsRate >= 20) {
          ...
          insights.push({ icon: '💰', text: `Saving ${savingsRate}% of income — consider increasing to 20%`, color: 'var(--fc-text-muted)', bg: 'var(--fc-bg-elevated-2)' });
        }
      }
    }
    ...
    if (state.period === '1M' && periodSpend > 0) {
      ...
      const projColor   = severeProj ? 'var(--fc-danger)' : overProj ? 'var(--fc-warning)' : 'var(--fc-text-muted)';
      const projBg      = severeProj ? 'rgba(255,69,58,0.08)' : overProj ? 'rgba(255,176,32,0.07)' : 'var(--fc-bg-elevated-2)';
      insights.push({
        icon: severeProj ? '🚨' : '📆',
    ...
    if (subs.length > 0) {
      ...
      insights.push({ icon: '📱', text: `${subs.length} active subscriptions — ${FCData.formatCurrency(subTotal)}/mo in recurring charges`, color: 'var(--fc-text-muted)', bg: 'var(--fc-bg-elevated-2)' });
    }
      ...
      for (const [, data] of Object.entries(incomeByKey)) {
        ...
        if (daysUntil >= 0 && daysUntil <= 14) {
          ...
          paydayInsight = {
            ...
            color: daysUntil <= 2 ? 'var(--fc-success)' : 'var(--fc-text-muted)',
            bg:   daysUntil <= 2 ? 'rgba(52,199,89,0.09)' : 'var(--fc-bg-elevated-2)',
    ...
    if (insights.length === 0) {
      insights.push({ icon: '🔗', text: 'Connect a bank account to unlock personalized insights', color: 'var(--fc-text-faint)', bg: 'var(--fc-bg-elevated-2)' });
    }
  ...
  function _renderHomeHealthScore(score, monthIncome, monthSpend, unpaidBills, overdueCount) {
    ...
    if (!_isPro()) {
      if (numEl) { numEl.textContent = 'PRO'; numEl.style.color = 'var(--fc-accent)'; }
      if (homeRing) homeRing.style.setProperty('--health-score', '18');
      if (labelEl) labelEl.textContent = 'Pro feature';
      if (subEl) subEl.textContent = 'Upgrade to calculate your score';
      return;
    }

    if (!state.user || !state.user.plaid_linked) {
      if (numEl)   numEl.textContent  = '—';
      if (homeRing) homeRing.style.setProperty('--health-score', '0');
    ...
    const normalizedScore = Math.round(score / 8.5);
    if (numEl)   { numEl.textContent = normalizedScore; numEl.style.color = color; }
    if (homeRing) homeRing.style.setProperty('--health-score', String(normalizedScore));
  ...
  function _renderPremiumHomeChart(netWorth) {
    const svg = document.getElementById('home-balance-chart');
    if (!svg) return;
    const deltaEl = document.getElementById('hero-delta');

    const history = state.nwHistory || {};
    const periodDays = { '1W': 7, '1M': 30, '3M': 90, '1Y': 365, 'ALL': 0 }[state.period] ?? 30;
    const cutoff = new Date();
    if (periodDays) cutoff.setDate(cutoff.getDate() - periodDays);
    const cutoffKey = cutoff.toISOString().slice(0, 10);
    let values = Object.keys(history).sort()
      .filter(key => !periodDays || key >= cutoffKey)
      .map(key => Number(history[key]))
      .filter(Number.isFinite);

    const email = (FCAuth.currentUser?.()?.email || state.user?.email || '').toLowerCase();
    if (values.length < 2 && _DEMO_EMAILS.includes(email)) {
      values = [0.86,0.875,0.87,0.895,0.91,0.905,0.93,0.945,0.97,0.965,0.99,1].map(scale => netWorth * scale);
    } else if (values.length < 2) {
      values = [netWorth, netWorth];
    }
    const delta = values[values.length - 1] - values[0];
    if (deltaEl) {
      deltaEl.textContent = `${delta >= 0 ? '+' : '-'}${FCData.formatCurrency(Math.abs(delta))}`;
      deltaEl.style.display = Math.abs(delta) > 0.5 ? '' : 'none';
      deltaEl.style.color = delta >= 0 ? 'var(--fc-success)' : 'var(--fc-danger)';
      deltaEl.style.background = delta >= 0 ? 'var(--fc-success-soft)' : 'var(--fc-danger-soft)';
    }
    const width = 320, height = 86, pad = 5;
    ...
    for (let i = 1; i < points.length; i++) {
      const prev = points[i - 1], point = points[i], middle = (prev.x + point.x) / 2;
      line += ` C${middle.toFixed(1)},${prev.y.toFixed(1)} ${middle.toFixed(1)},${point.y.toFixed(1)} ${point.x.toFixed(1)},${point.y.toFixed(1)}`;
    }
    const last = points[points.length - 1];
    svg.innerHTML = `<defs><linearGradient id="premiumHomeArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="var(--fc-accent)" stop-opacity=".30"/><stop offset="100%" stop-color="var(--fc-accent)" stop-opacity="0"/></linearGradient></defs><path d="${line} L${width},${height} L0,${height} Z" fill="url(#premiumHomeArea)"/><path d="${line}" fill="none" stroke="var(--fc-accent)" stroke-width="2.5" stroke-linecap="round"/><circle cx="${last.x.toFixed(1)}" cy="${last.y.toFixed(1)}" r="4" fill="var(--fc-accent)" stroke="var(--fc-bg-elevated)" stroke-width="2"/>`;
  }
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px">
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
            ...
            <span style="font-size:10px;color:var(--fc-text-faint);margin-left:auto;font-weight:500">Partner</span>
          </div>
      ...
      return `
        <div>
          ...
          <div style="height:6px;position:relative;margin-top:2px">
            <div style="position:absolute;left:${target}%;top:-8px;width:1px;height:16px;background:var(--fc-border-strong)"></div>
          </div>
        </div>`;
    }
  ...
  function _renderNetWorthMilestone(netWorth) {
    ...
    const historyEntries = Object.entries(state.nwHistory || {})
      .filter(([, value]) => Number.isFinite(Number(value)))
      .sort(([a], [b]) => a.localeCompare(b));
    const historyValues = historyEntries.map(([, value]) => Number(value));

    // Remove sensitive values written by pre-v3 builds. Deletion is the only
    // localStorage access allowed in this financial feature.
    if (!_legacyFinancialStorageCleaned) {
      try {
        Object.keys(localStorage)
          .filter(key => key === 'fc_debt_start' || key.startsWith('fc_milestone_'))
          .forEach(key => localStorage.removeItem(key));
      } catch (_) {}
      _legacyFinancialStorageCleaned = true;
    }

    // ── Debt payoff mode — negative net worth ─────────────────────────────
    if (netWorth < 0) {
      // Derive the lowest point from Firestore-backed history instead of
      // persisting a financial value in browser storage.
      const negativeHistory = historyValues.filter(value => value < 0);
      const debtStart = Math.min(netWorth, ...(negativeHistory.length ? negativeHistory : [netWorth]));
    ...
    // Compare with the previous Firestore snapshot. The old implementation
    // compared against `next` (which is always above netWorth), so milestone
    // celebrations could never fire.
    const today = new Date().toISOString().split('T')[0];
    const priorValues = historyEntries
      .filter(([date]) => date < today)
      .map(([, value]) => Number(value));
    const previousNW = priorValues.length ? priorValues[priorValues.length - 1] : netWorth;
    const crossed = [...MILESTONES].reverse().find(m => previousNW < m && netWorth >= m);
    const celebrated = Array.isArray(state.user?.milestones_seen)
      ? state.user.milestones_seen.map(Number)
      : [];
    const justCrossed = Number.isFinite(crossed) && !celebrated.includes(crossed);

    let showBadge = false;
    if (justCrossed) {
      showBadge = true;
      const milestonesSeen = [...new Set([...celebrated, crossed])].sort((a, b) => a - b);
      if (state.user) state.user.milestones_seen = milestonesSeen;
      FCData.updateUserField('milestones_seen', milestonesSeen).catch(() => {});
      haptic('success');
      // Confetti burst
    ...
    // Update text
    if (titleEl) titleEl.textContent = showBadge ? `You hit ${fmtMs(crossed)}!` : `On the way to ${nextLabel}`;
  ...
  function _renderAccountRows() {
    ...
    // Accounts section is not shown on Home — managed via Wealth tab
    ...
    const typeColor = t => ({ depository:'rgba(55,138,221,0.18)', savings:'rgba(29,158,117,0.18)', credit:'rgba(216,90,48,0.18)', investment:'rgba(96,165,250,0.18)', loan:'rgba(255,159,10,0.18)', mortgage:'rgba(255,159,10,0.18)' }[t] || 'var(--fc-bg-elevated-2)');
      ...
      return `
        <div class="fc-home-txn" onclick="FCApp.openTransactionDetail('${esc(t.id)}')" role="button" tabindex="0">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div class="fc-txn-meta">${esc(cat)}</div>
          </div>
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:2px;flex-shrink:0">
            <div class="fc-txn-amt${isCredit ? ' fc-txn-credit' : ''}">${isCredit ? '+' : '−'}${FCData.formatCurrency(amt)}</div>
            <div style="font-size:11px;color:var(--fc-text-faint);font-variant-numeric:tabular-nums">${esc(dateStr)}</div>
          </div>
        </div>`;
    }).join('');
  }
  ...
  /* ─────────────────────────────────────────────────────────────
     ...
     ───────────────────────────────────────────────────────────── */
  function _renderThisMonthsMove() {
    const section = document.getElementById('this-months-move-section');
    if (!section) return;

    const insights = _buildFocusInsights ? _buildFocusInsights() : [];
    // Pick the highest-priority non-"good week" insight, or fall back to motivational
    const pick = insights.find(i => i.tier !== 'good_week' && i.tier !== 'fallback') || insights[0];
    if (!pick) { section.style.display = 'none'; return; }

    const card = section.querySelector('.dash-move-card');
    if (!card) return;
    section.style.display = '';

    const iconWrap = card.querySelector('.dash-move-icon-wrap');
    const bodyText = card.querySelector('.dash-move-text');
    const ctaBtn   = card.querySelector('.dash-move-cta');
    if (iconWrap) iconWrap.textContent = pick.icon || '⭐';
    if (bodyText) bodyText.textContent = pick.body || pick.text || '';
    if (ctaBtn) {
      ctaBtn.onclick = () => {
        if (pick.tab) FCApp.switchTab(pick.tab);
        else if (typeof FCApp._focusCardTap === 'function') { _focusIdx = insights.indexOf(pick); FCApp._focusCardTap(); }
      };
    }
    card.onclick = () => {
      if (pick.tab) FCApp.switchTab(pick.tab);
  ...
  function _buildCashFlowForecast() {
    const cash  = FCData.calcCash(state.accounts);
    const now   = new Date();
    const bills7 = _getBillsDueInDays(7);
    const billsTotal = bills7.reduce((s, b) => s + (b.amount || 0), 0);

    const cut14 = new Date(now.getTime() - 14 * 86400000);
    const inc14 = (state.transactions || [])
      .filter(t => _isIncomeTxn(t) && FCData.parseDateLocal(t.date) >= cut14)
      .reduce((s, t) => s + Math.abs(t.amount || 0), 0);
    const expectedIncome = inc14 > 0 ? inc14 * 0.5 : 0;

    const cut30 = new Date(now.getTime() - 30 * 86400000);
    const spend30 = (state.transactions || [])
      .filter(t => _isSpendTxn(t) && FCData.parseDateLocal(t.date) >= cut30)
      .reduce((s, t) => s + (t.amount || 0), 0);
    const otherSpend = spend30 > 0 ? (spend30 / 30) * 7 - billsTotal : 0;
    const otherSpendAdj = Math.max(0, otherSpend);

    return { cash, expectedIncome, billsTotal, otherSpend: otherSpendAdj, projected: cash + expectedIncome - billsTotal - otherSpendAdj, bills7 };
  }

  function _getYesterdayChanges() {
    const now = new Date();
    const yesterday = new Date(now.getTime() - 24 * 3600000);

    const recent = (state.transactions || []).filter(t =>
      FCData.parseDateLocal(t.date) >= yesterday
    );
    const cashDelta = recent.reduce((s, t) =>
      s + (t.isCredit ? Math.abs(t.amount || 0) : -(t.amount || 0)), 0
    );
    const catSpend = {};
    recent.filter(_isSpendTxn).forEach(t => {
      const cat = FCData.normalizePlaidCategory((t.category && t.category[0]) || 'Other');
    ...
    const noUpcomingBills = !nextBill || (FCData.daysUntil(nextBill.due_date) ?? 999) > 7;
    return { cashDelta, txnCount: recent.length, topCatEntry, noUpcomingBills };
  }

  /* ── Today's Money Move ──────────────────────────────────────── */
  function _renderHomeMoveCard() {
    const section = document.getElementById('home-move-section');
    if (!section) return;
    if (!state.user?.plaid_linked) {
      section.style.display = 'none'; return;
    }
    const insights = _buildFocusInsights();
    let pick = insights[0];

    // When no urgent issue exists, turn the card into a concrete wealth-building
    // action instead of restating a balance or congratulating the user.
    if (!pick || pick.fallback) {
      const emergencyGoal = (state.goals || []).find(g => /emergency|rainy|reserve/i.test(g.name || ''));
      if (emergencyGoal) {
        const remaining = Math.max(0, (emergencyGoal.target || 0) - (emergencyGoal.current || 0));
        const contribution = Math.min(50, remaining || 50);
        pick = {
          type: 'good',
          title: `Add ${FCData.formatCurrency(contribution)} to ${emergencyGoal.name}`,
          body: remaining > contribution
            ? `A small contribution keeps your safety net moving without putting upcoming bills at risk.`
            : `This contribution completes your ${emergencyGoal.name} goal.`,
          action: `Add ${FCData.formatCurrency(contribution)}`,
          healthLift: 3,
          tap: async () => {
            try {
              await FCData.updateGoal(emergencyGoal.id, {
                current: Math.min(emergencyGoal.target || Infinity, (emergencyGoal.current || 0) + contribution)
              });
              haptic('success');
              toast(`${FCData.formatCurrency(contribution)} added to ${emergencyGoal.name}`, 'success');
            } catch (_) {
              toast('Could not update this goal. Please try again.', 'error');
            }
          }
        };
      } else {
        pick = {
          type: 'info',
          title: 'Move $50 to Emergency Fund',
          body: 'You’ll stay on track and start building a buffer for unexpected expenses.',
          action: 'Move $50',
          healthLift: 3,
          tap: () => FCApp.showAddGoalSheet()
        };
      }
    }
    section.style.display = '';

    const card    = document.getElementById('home-move-card');
    const titleEl = document.getElementById('home-move-title');
    const subEl   = document.getElementById('home-move-sub');
    const doBtn   = document.getElementById('home-move-do');
    if (titleEl) titleEl.textContent = pick.title || pick.label || '';
    if (subEl)   subEl.textContent   = pick.body  || '';
    if (card) {
      card.classList.toggle('danger-card', pick.type === 'danger');
      card.classList.toggle('warn-card', pick.type === 'warn');
      card.dataset.tone = pick.type || 'info';
    }
    if (doBtn) {
      const isDanger = pick.type === 'danger';
      const isWarn   = pick.type === 'warn';
      const isGood   = pick.type === 'good';
      doBtn.style.background = isDanger ? 'var(--fc-danger)' : isWarn ? 'var(--fc-warning)' : isGood ? 'var(--fc-accent)' : 'var(--fc-electric)';
      doBtn.textContent = pick.action || 'View';
      doBtn.onclick = () => { haptic('medium'); if (pick.tap) pick.tap(); };
    }

    const now = new Date();
    const monthTxns = (state.transactions || []).filter(t => t.date && FCData.isCurrentMonth(t.date));
    const monthSpend = monthTxns.filter(_isSpendTxn).reduce((sum, t) => sum + (t.amount || 0), 0);
    const monthIncome = monthTxns.filter(_isIncomeTxn).reduce((sum, t) => sum + (t.amount || 0), 0);
    ...
    const currentHealth = Math.round(_calcHealthScore(monthIncome, monthSpend, unpaidBills, overdueCount) / 8.5);
    const defaultLift = pick.type === 'danger' ? 4 : pick.type === 'warn' ? 3 : pick.type === 'good' ? 3 : 2;
    const nextHealth = Math.min(100, currentHealth + (pick.healthLift || defaultLift));
    const healthCurrentEl = document.getElementById('home-move-health-current');
    const healthNextEl = document.getElementById('home-move-health-next');
    if (healthCurrentEl) healthCurrentEl.textContent = currentHealth;
    if (healthNextEl) healthNextEl.textContent = nextHealth;
  }

  function _getSpendableCheckingCash() {
    const checking = (state.accounts || []).filter(a => {
      const type = String(a.type || '').toLowerCase();
      const subtype = String(a.subtype || '').toLowerCase();
      return type === 'depository' && !/savings|money market|cd/.test(subtype);
    });
    const source = checking.length ? checking : (state.accounts || []).filter(a => a.type === 'depository');
    return source.reduce((sum, account) => sum + (account.balance_current || account.balance || 0), 0);
  }
  ...
  function _renderHomeForecast() {
    const section  = document.getElementById('home-forecast-section');
    const card     = document.getElementById('home-forecast-card');
    ...
    const fc = _buildCashFlowForecast();
    const isRisk    = fc.projected < 500 && fc.projected >= 0;
    const isDanger  = fc.projected < 0;
    const statusColor = isDanger ? 'var(--fc-danger)' : isRisk ? 'var(--fc-warning)' : 'var(--fc-success)';
    const statusText  = isDanger ? 'Cash flow negative' : isRisk ? 'Running low' : 'All clear';
    const statusIcon  = isDanger
      ? `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`
      : isRisk
      ? `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>`
    ...
    // Build 8 sample points (today + 7 days) for sparkline
    const pts = [];
    {
      let bal = fc.cash;
      const perDay = { income: fc.expectedIncome / 7, bills: fc.billsTotal / 7, other: fc.otherSpend / 7 };
      for (let d = 0; d <= 7; d++) {
        pts.push(Math.max(0, d === 0 ? fc.cash : bal));
        bal += perDay.income - perDay.bills - perDay.other;
      }
    }
    const ptMin = Math.min(...pts);
    const ptMax = Math.max(...pts) || 1;
    const ptRange = ptMax - ptMin || 1;
    const svgW = 100, svgH = 40, padT = 4, padB = 4;
    const drawH = svgH - padT - padB;
    const svgCoords = pts.map((v, i) => ({
      x: (i / 7) * svgW,
      y: padT + drawH - ((v - ptMin) / ptRange) * drawH
    }));
    const linePath = svgCoords.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
    const fillPath = linePath + ` L${svgW},${svgH} L0,${svgH} Z`;
    const lineColor = isDanger ? '#ef4444' : isRisk ? '#f59e0b' : '#1ac4f0';
    const fillId = 'fcFcastGrad_' + Math.random().toString(36).slice(2, 6);

    const billsRow = fc.billsTotal > 0
      ? `<div class="home-forecast-row"><span class="home-forecast-label">Bills due</span><span class="home-forecast-val" style="color:var(--fc-danger)">−${FCData.formatCurrency(fc.billsTotal)}</span></div>`
      : '';
    const otherRow = fc.otherSpend > 5
      ? `<div class="home-forecast-row"><span class="home-forecast-label">${fc.expectedIncome > 0 ? 'Other est.' : 'Est. spend'}</span><span class="home-forecast-val" style="color:var(--fc-text-muted)">−${FCData.formatCurrency(fc.otherSpend)}</span></div>`
      : '';
    const incomeRow = fc.expectedIncome > 0
      ? `<div class="home-forecast-row"><span class="home-forecast-label">Expected income</span><span class="home-forecast-val" style="color:var(--fc-success)">+${FCData.formatCurrency(fc.expectedIncome)}</span></div>`
      : '';
    const rows = incomeRow + billsRow + otherRow;

    card.innerHTML = `
      <div class="home-forecast-header">
        <span class="home-forecast-label" style="font-size:10px;letter-spacing:.06em;text-transform:uppercase">7-Day Outlook</span>
        <span class="home-forecast-status ${isDanger ? 'bad' : isRisk ? 'warn' : 'good'}">${statusText}</span>
      </div>
      <div class="home-forecast-chart-wrap">
        <svg class="home-forecast-chart-svg" viewBox="0 0 ${svgW} ${svgH}" preserveAspectRatio="none" aria-hidden="true">
          <defs>
          ...
          <path d="${linePath}" fill="none" stroke="${lineColor}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
          <circle cx="${svgCoords[0].x.toFixed(1)}" cy="${svgCoords[0].y.toFixed(1)}" r="2.5" fill="${lineColor}" opacity=".9"/>
          <circle cx="${svgCoords[7].x.toFixed(1)}" cy="${svgCoords[7].y.toFixed(1)}" r="2.5" fill="${lineColor}" opacity=".9"/>
        </svg>
      </div>
      ${rows}
      <div class="home-forecast-projected">
        <span class="home-forecast-proj-label">Projected Balance</span>
        <span class="home-forecast-proj-val" style="color:${statusColor}">${FCData.formatCurrency(Math.max(0, fc.projected))}</span>
      </div>`;
  }

  /* ── What Changed Since Yesterday ──────────────────────────── */
  function _renderHomeYesterday() {
    const section = document.getElementById('home-yesterday-section');
    const grid    = document.getElementById('home-yesterday-grid');
    if (!section || !grid) return;
    if (!state.user?.plaid_linked || !state.transactions?.length) {
      section.style.display = 'none'; return;
    }
    section.style.display = '';

    const ch = _getYesterdayChanges();
    const tiles = [];

    const _svgCash = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg>`;
    const _svgTxn  = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`;
    const _svgCat  = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>`;
    const _svgBill = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`;
    const _svgOk   = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>`;

    const cashSign  = ch.cashDelta >= 0 ? '+' : '−';
    const cashColor = ch.cashDelta >= 0 ? 'var(--fc-success)' : 'var(--fc-danger)';
    const cashBg    = ch.cashDelta >= 0 ? 'rgba(48,209,88,0.12)' : 'rgba(255,69,58,0.12)';
    tiles.push({
      icon: _svgCash, iconBg: cashBg, iconColor: cashColor,
      value: `${cashSign}${FCData.formatCurrency(Math.abs(ch.cashDelta))}`,
      label: 'Cash · 24h', color: cashColor
    });

    const txnBg = ch.txnCount === 0 ? 'rgba(48,209,88,0.12)' : 'rgba(26,196,240,0.12)';
    const txnColor = ch.txnCount === 0 ? 'var(--fc-success)' : 'var(--fc-accent)';
    tiles.push({
      icon: _svgTxn, iconBg: txnBg, iconColor: txnColor,
      value: ch.txnCount === 0 ? '0' : String(ch.txnCount),
      label: ch.txnCount === 0 ? 'No activity' : `Transaction${ch.txnCount !== 1 ? 's' : ''}`,
      color: txnColor
    });

    if (ch.topCatEntry) {
      const [cat, amt] = ch.topCatEntry;
      tiles.push({
        icon: _svgCat, iconBg: 'rgba(255,159,10,0.12)', iconColor: 'var(--fc-warning)',
        value: FCData.formatCurrency(amt), label: cat, color: 'var(--fc-warning)'
      });
    } else {
      tiles.push({
        icon: _svgOk, iconBg: 'rgba(48,209,88,0.12)', iconColor: 'var(--fc-success)',
        value: '—', label: 'No spending', color: 'var(--fc-success)'
      });
    }

    const billBg    = ch.noUpcomingBills ? 'rgba(48,209,88,0.12)' : 'rgba(255,159,10,0.12)';
    const billColor = ch.noUpcomingBills ? 'var(--fc-success)' : 'var(--fc-warning)';
    tiles.push({
      icon: ch.noUpcomingBills ? _svgOk : _svgBill,
      iconBg: billBg, iconColor: billColor,
      value: ch.noUpcomingBills ? '✓' : '1',
      label: ch.noUpcomingBills ? 'Bills clear' : 'Bill due soon',
      color: billColor
    });

    grid.innerHTML = tiles.map(t =>
      `<div class="home-yesterday-tile">
        <div class="home-ytile-icon-wrap" style="background:${t.iconBg};color:${t.iconColor}">${t.icon}</div>
        <div class="home-ytile-val" style="color:${t.color}">${t.value}</div>
        <div class="home-ytile-label">${t.label}</div>
      </div>`
    ).join('');
  }

  /* ── Subscriptions to Review (Home) ────────────────────────── */
  function _renderHomeSubsCard() {
    const section  = document.getElementById('home-subs-section');
    const titleEl  = document.getElementById('home-subs-title');
    const subEl    = document.getElementById('home-subs-sub');
    const badgeEl  = document.getElementById('home-subs-badge');
    if (!section) return;
    if (!state.user?.plaid_linked || !state.transactions?.length) {
      section.style.display = 'none'; return;
    }
    try {
      const zombies = _detectSubscriptions().filter(s => !s.tracked);
      if (!zombies.length) { section.style.display = 'none'; return; }
      const total = zombies.reduce((s, z) => s + (z.amount || 0), 0);
      section.style.display = '';
      if (titleEl) titleEl.textContent = `You could save ${FCData.formatCurrency(total)}/month`;
      if (subEl)   subEl.textContent   = `${zombies.length} recurring charge${zombies.length !== 1 ? 's' : ''} to review`;
      if (badgeEl) { badgeEl.textContent = zombies.length; badgeEl.style.display = 'flex'; }
  ...
  function _renderHome() {
    ...
    _renderPremiumHomeChart(netWorth);
    ...
    const overdueCount     = unpaidBills.filter(b => (FCData.daysUntil(b.due_date) ?? 1) < 0).length;
    _renderHomeHealthScore(
      _calcHealthScore(monthIncome, monthSpend, unpaidBills, overdueCount),
      monthIncome,
      monthSpend,
      unpaidBills,
      overdueCount
  ...
  /* ─────────────────────────────────────────────────────────────
     ...
     ───────────────────────────────────────────────────────────── */
  function _renderHomeDashboardLegacy() {
    const el = document.getElementById('home-dash');
    if (!el) return;

    const user      = state.user || {};
    const firstName = (user.name || 'there').split(' ')[0].replace(/[.\d]/g, '') || 'there';
    const now       = new Date();
    ...
    const isLinked     = !!(user.plaid_linked || _isDemoMode);

    // Month figures
    const mStart      = new Date(now.getFullYear(), now.getMonth(), 1);
    const monthTxns   = transactions.filter(t => t.date && FCData.parseDateLocal(t.date) >= mStart);
    const monthIncome = monthTxns.filter(_isIncomeTxn).reduce((s,t) => s+(t.amount||0), 0);
    const monthSpend  = monthTxns.filter(_isSpendTxn).reduce((s,t) => s+(t.amount||0), 0);
    const cashFlow    = monthIncome - monthSpend;

    // Safe to spend
    let safeToSpend = 0;
    if (isLinked) {
      try { safeToSpend = Math.max(0, _buildSafeSpendProjection().safe); } catch(_) {}
    }
    ...
    // Budget
    const budgetLimit = budgets['total']?.limit || 0;
    const budgetPct   = budgetLimit > 0 ? Math.min(Math.round(monthSpend/budgetLimit*100),100) : 0;
    const budgetColor = budgetPct > 90 ? 'var(--fc-danger)' : budgetPct > 70 ? 'var(--fc-warning)' : 'var(--fc-accent)';

    // Top spending category
    const catMap = {};
    monthTxns.filter(_isSpendTxn).forEach(t => {
      const c = (t.category?.[1]||t.category?.[0]||'Other');
      catMap[c] = (catMap[c]||0)+(t.amount||0);
    });
    const topCat = Object.entries(catMap).sort((a,b)=>b[1]-a[1])[0];

    // Financial Freedom Score (0–100)
    const unpaidBills  = bills.filter(b => b.status !== 'paid');
    const overdueCount = unpaidBills.filter(b => (FCData.daysUntil(b.due_date)??1)<0).length;
    const healthScore  = Math.min(100, Math.round(_calcHealthScore(monthIncome, monthSpend, unpaidBills, overdueCount)/8.5));
    const healthColor  = healthScore >= 70 ? 'var(--fc-accent)' : healthScore >= 50 ? 'var(--fc-warning)' : 'var(--fc-danger)';
    const healthMsg    = healthScore >= 80 ? "You're building great habits. Keep it up!"
                       : healthScore >= 60 ? "You're on track. Small wins add up."
                       : healthScore >= 40 ? "Room to improve — focus on saving more."
                       : "Let's build better habits together.";

    // Recent transactions (3)
    const recentTxns = [...transactions].sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,3);

    // "Your Next Best Move" hero card
    const efGoal = goals.find(g => /emergency|rainy|reserve/i.test(g.name||''));
    let moveTitleText, moveBodyText, moveBtnLabel, moveBtnAction;
    if (safeToSpend > 50 && efGoal && (efGoal.pct||0) < 100) {
      const moveAmt = Math.min(50, Math.round(safeToSpend * 0.15));
      moveTitleText = 'You have ' + FCData.formatCurrency(safeToSpend) + ' extra before payday.';
      moveBodyText  = 'Move ' + FCData.formatCurrency(moveAmt) + ' to ' + (efGoal.name||'Emergency Fund') + '.';
      moveBtnLabel  = 'Move ' + FCData.formatCurrency(moveAmt) + ' Now';
      moveBtnAction = "_openSubScreen('goals')";
    } else if (budgetLimit > 0 && monthSpend > budgetLimit && topCat) {
      const over = monthSpend - budgetLimit;
      moveTitleText = 'Budget exceeded by ' + FCData.formatCurrency(over) + '.';
      moveBodyText  = topCat[0] + ' is your biggest spend this month.';
      moveBtnLabel  = 'Review Spending';
      moveBtnAction = "switchTab('activity')";
    } else {
      let ins = [];
      try { ins = _buildFocusInsights ? _buildFocusInsights() : []; } catch(_) {}
      const first = ins[0];
      moveTitleText = (first && first.title) || 'Review your financial plan.';
      moveBodyText  = (first && first.body)  || 'Stay on top of bills, goals & budget.';
      moveBtnLabel  = 'Review Plan';
      moveBtnAction = "switchTab('plan')";
    }

    // Auth user initial for header avatar
    const authUser3 = window.FCAuth && FCAuth.currentUser ? FCAuth.currentUser() : null;
    const rawName3  = (user.name||authUser3?.displayName||'').trim();
    const initial   = rawName3.charAt(0).toUpperCase() || '?';
    const hdrAvatar = document.getElementById('header-avatar-initial');
    if (hdrAvatar) hdrAvatar.textContent = initial;

    // ── Helpers ──────────────────────────────────────────────────
    const fmt = v => FCData.formatCurrency(v);

    function txnDateLbl(ds) {
      if (!ds) return '';
      const d = FCData.parseDateLocal(ds);
    ...
    function billDueFmt(b) {
      ...
      try {
        const d = FCData.parseDateLocal ? FCData.parseDateLocal(b.due_date) : new Date(b.due_date+'T12:00:00');
        return 'Due '+d.toLocaleDateString('en-US',{month:'short',day:'numeric'});
      } catch(_) { return 'Due in '+days+'d'; }
    }

    function billDayColor(b) {
      const days = FCData.daysUntil(b.due_date);
      if (days===null||days>6) return 'var(--fc-text-muted)';
      if (days<0) return 'var(--fc-danger)';
      if (days<=3) return 'var(--fc-warning)';
      return 'var(--fc-text-muted)';
    }

    function txnIcon(t) {
      const j = (t.category||[]).join(' ').toLowerCase()+' '+(t.name||'').toLowerCase();
      if (t.isCredit) return '💰';
      if (j.includes('amazon')) return '📦';
      if (j.includes('starbucks')||j.includes('coffee')) return '☕';
      if (j.includes('target')||j.includes('walmart')) return '🛒';
      if (j.includes('restaurant')||j.includes('chipotle')||j.includes('dining')) return '🍽️';
      if (j.includes('uber')||j.includes('lyft')||j.includes('ride')) return '🚗';
      if (j.includes('gas')||j.includes('fuel')) return '⛽';
      if (j.includes('netflix')||j.includes('spotify')||j.includes('subscription')) return '🎬';
      return '💳';
    }

    function txnBg(t) {
      if (t.isCredit) return 'rgba(34,197,94,0.12)';
      const j = (t.category||[]).join(' ').toLowerCase();
      if (j.includes('food')||j.includes('restaurant')||j.includes('coffee')) return 'rgba(249,115,22,0.10)';
      if (j.includes('shop')||j.includes('amazon')) return 'rgba(59,130,246,0.10)';
      if (j.includes('travel')||j.includes('uber')) return 'var(--fc-electric-soft)';
      return 'rgba(100,116,139,0.10)';
    }

    function goalEmoji(g) {
      const n = (g.name||'').toLowerCase();
      if (n.includes('emergency')||n.includes('rainy')) return '🛡️';
      if (n.includes('vacation')||n.includes('travel')) return '🌴';
      if (n.includes('home')||n.includes('down')) return '🏠';
    ...
    function bar(pct, color, h) {
      const t = h||5;
      return '<div style="height:'+t+'px;background:rgba(13,27,46,0.07);border-radius:'+t+'px;overflow:hidden"><div style="height:100%;width:'+Math.min(pct,100)+'%;background:'+color+';border-radius:'+t+'px;transition:width .5s"></div></div>';
    }

    function donutSVG(pct, color) {
      const r=38,cx=48,cy=48,circ=2*Math.PI*r;
      return '<svg width="96" height="96" viewBox="0 0 96 96"><circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="rgba(13,27,46,0.07)" stroke-width="8"/><circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="'+color+'" stroke-width="8" stroke-dasharray="'+circ.toFixed(1)+'" stroke-dashoffset="'+(circ*(1-pct/100)).toFixed(1)+'" stroke-linecap="round" transform="rotate(-90 '+cx+' '+cy+')"/></svg>';
    }

    const card = (p) => 'background:var(--fc-bg-elevated);border:1px solid var(--fc-border);border-radius:20px;box-shadow:0 2px 12px rgba(13,27,46,0.06);padding:'+(p||'18px 20px');
    const eyebrow = 'font-size:10px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:var(--fc-text-faint)';
    function linkBtn(action, label) { return '<button onclick="'+action+'" style="font-size:12px;font-weight:600;color:var(--fc-accent);background:none;border:none;cursor:pointer;padding:0;font-family:inherit;-webkit-tap-highlight-color:transparent">'+label+'</button>'; }
    const goalBarColors = ['var(--fc-accent)','var(--fc-success)','var(--fc-warning)'];
    const goalBgColors  = ['var(--fc-accent-soft)','var(--fc-success-soft)','var(--fc-warning-soft)'];

    // ── Render ────────────────────────────────────────────────────
    const syncBadge = isLinked ? '<div style="display:inline-flex;align-items:center;gap:5px;background:var(--fc-success-soft);border:1px solid rgba(34,197,94,0.22);border-radius:999px;padding:5px 12px;flex-shrink:0;margin-top:4px"><div style="width:7px;height:7px;border-radius:50%;background:var(--fc-success);flex-shrink:0"></div><span style="font-size:12px;font-weight:600;color:var(--fc-success)">Synced</span></div>' : '';

    const nextBillHtml = nextBill
      ? '<div style="font-size:13px;font-weight:600;color:var(--fc-text);margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(nextBill.name)+'</div>'
        +'<div style="font-size:22px;font-weight:700;color:var(--fc-text);letter-spacing:-0.03em;font-variant-numeric:tabular-nums;margin-bottom:4px">'+fmt(nextBill.amount)+'</div>'
        +'<div style="font-size:12px;color:'+billDayColor(nextBill)+';margin-bottom:8px">'+esc(billDueFmt(nextBill))+'</div>'
        +linkBtn("FCApp.switchTab('plan')", 'View all bills ›')
      : '<div style="font-size:13px;font-weight:600;color:var(--fc-success);margin-bottom:4px">All clear ✓</div>'
        +'<div style="font-size:12px;color:var(--fc-text-muted)">No bills due soon</div>';

    const cfArrow = cashFlow >= 0
      ? '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--fc-success)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>'
      : '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--fc-danger)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>';

    const budgetHtml = budgetLimit > 0
      ? '<div style="display:flex;align-items:center;gap:18px">'
        +'<div style="position:relative;flex-shrink:0">'+donutSVG(budgetPct,budgetColor)+'<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center"><span style="font-size:20px;font-weight:700;color:var(--fc-text);line-height:1">'+budgetPct+'%</span></div></div>'
        +'<div style="flex:1;min-width:0">'
        +'<div style="font-size:15px;font-weight:700;color:var(--fc-text);font-variant-numeric:tabular-nums;margin-bottom:4px">'+fmt(monthSpend)+' <span style="font-size:13px;font-weight:400;color:var(--fc-text-muted)">of '+fmt(budgetLimit)+'</span></div>'
        +(topCat && monthSpend>budgetLimit ? '<div style="font-size:13px;color:var(--fc-danger);font-weight:600;margin-bottom:8px">'+esc(topCat[0])+' is over.</div>' : topCat ? '<div style="font-size:12px;color:var(--fc-text-muted);margin-bottom:8px">Top: '+esc(topCat[0])+'</div>' : '<div style="margin-bottom:8px"></div>')
        +linkBtn("FCApp.switchTab('plan')", 'View budget ›')
        +'</div></div>'
      : '<div style="display:flex;align-items:center;gap:18px">'
        +'<div style="position:relative;flex-shrink:0">'+donutSVG(0,'rgba(13,27,46,0.12)')+'<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center"><span style="font-size:11px;font-weight:600;color:var(--fc-text-muted);text-align:center;line-height:1.3">No<br>budget</span></div></div>'
        +'<div style="flex:1;min-width:0">'
        +'<div style="font-size:14px;font-weight:600;color:var(--fc-text);margin-bottom:5px">Create your budget</div>'
        +'<div style="font-size:12px;color:var(--fc-text-muted);margin-bottom:10px">Track spending against a monthly plan.</div>'
        +linkBtn("FCApp._openBudgetWizard&&FCApp._openBudgetWizard()", 'Create budget ›')
        +'</div></div>';

    const goalsHtml = goals.length
      ? goals.slice(0,3).map((g,i) =>
          '<div style="display:flex;align-items:center;gap:12px;padding:10px 0;'+(i<Math.min(goals.length,3)-1?'border-bottom:1px solid var(--fc-border)':'')+'">'
          +'<div style="width:36px;height:36px;border-radius:10px;background:'+goalBgColors[i%3]+';display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">'+goalEmoji(g)+'</div>'
          +'<div style="flex:1;min-width:0">'
          +'<div style="font-size:13px;font-weight:600;color:var(--fc-text);margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(g.name||'Goal')+'</div>'
          +bar(Math.min(g.pct||0,100),goalBarColors[i%3])
          +'<div style="font-size:11px;color:var(--fc-text-muted);margin-top:3px">'+fmt(g.current||0)+' / '+fmt(g.target||0)+'</div>'
          +'</div>'
          +'<div style="font-size:13px;font-weight:700;color:var(--fc-text-muted);flex-shrink:0;min-width:34px;text-align:right">'+Math.min(g.pct||0,100)+'%</div>'
          +'</div>'
        ).join('')
      : '<div style="text-align:center;padding:20px 0;color:var(--fc-text-muted);font-size:13px">No goals yet — '+linkBtn("FCApp._openSubScreen('goals')", 'add your first goal')+'</div>';

    const activityHtml = recentTxns.length
      ? recentTxns.map((t,i) =>
          '<div style="display:flex;align-items:center;gap:12px;padding:10px 0;'+(i<recentTxns.length-1?'border-bottom:1px solid var(--fc-border)':'')+';cursor:pointer;-webkit-tap-highlight-color:transparent" onclick="FCApp.openTransactionDetail&&FCApp.openTransactionDetail(\''+esc(t.id||'')+'\')">'
          +'<div style="width:38px;height:38px;border-radius:10px;background:'+txnBg(t)+';display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">'+txnIcon(t)+'</div>'
          +'<div style="flex:1;min-width:0">'
          +'<div style="font-size:13px;font-weight:600;color:var(--fc-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(t.name||'Transaction')+'</div>'
          +'<div style="font-size:11px;color:var(--fc-text-muted);margin-top:1px">'+esc((Array.isArray(t.category)?t.category[0]:t.category)||'')+'</div>'
          +'</div>'
          +'<div style="text-align:right;flex-shrink:0">'
          +'<div style="font-size:13px;font-weight:700;color:'+(t.isCredit?'var(--fc-success)':'var(--fc-text)')+';font-variant-numeric:tabular-nums">'+(t.isCredit?'+':'-')+fmt(t.amount||0)+'</div>'
          +'<div style="font-size:11px;color:var(--fc-text-muted);margin-top:1px">'+txnDateLbl(t.date)+'</div>'
          +'</div></div>'
        ).join('')
      : '<div style="text-align:center;padding:20px 0;color:var(--fc-text-muted);font-size:13px">No transactions yet</div>';

    const qaBtn = (action, iconPath, title, sub, iconBg, iconColor) =>
      '<button onclick="'+action+'" style="'+card('14px 12px')+';display:flex;align-items:center;gap:10px;cursor:pointer;font-family:inherit;border:1px solid var(--fc-border);-webkit-tap-highlight-color:transparent;text-align:left;width:100%;box-sizing:border-box" onmousedown="this.style.opacity=\'.75\'" onmouseup="this.style.opacity=\'1\'" ontouchstart="this.style.opacity=\'.75\'" ontouchend="this.style.opacity=\'1\'">'
      ...
      +'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="'+iconColor+'" stroke-width="2.5" stroke-linecap="round">'+iconPath+'</svg>'
      +'</div>'
      +'<div style="flex:1;min-width:0;text-align:left">'
      +'<div style="font-size:13px;font-weight:600;color:var(--fc-text);line-height:1.2">'+title+'</div>'
      +'<div style="font-size:11px;color:var(--fc-text-muted);margin-top:1px">'+sub+'</div>'
      +'</div>'
      +'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--fc-text-faint)" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg>'
      +'</button>';

    el.innerHTML =
      // 1. Greeting
      '<div style="display:flex;align-items:flex-start;justify-content:space-between;padding:14px 20px 8px;overflow:hidden">'
      +'<div>'
      +'<h1 style="font-size:26px;font-weight:700;color:var(--fc-text);letter-spacing:-0.025em;margin:0;line-height:1.2">'+esc(tod)+', '+esc(firstName)+'</h1>'
      +'<p style="font-size:13px;color:var(--fc-text-muted);margin:3px 0 0">'+esc(dateStr)+'</p>'
      +'</div>'+syncBadge+'</div>'

      // 2. Your Next Best Move
      +'<div style="margin:6px 16px 14px;'+card()+';border-color:rgba(20,124,255,0.18)">'
      +'<div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:14px">'
      +'<div style="flex:1;min-width:0">'
      +'<div style="'+eyebrow+';color:var(--fc-accent);margin-bottom:8px">YOUR NEXT BEST MOVE</div>'
      +'<p style="font-size:16px;font-weight:700;color:var(--fc-text);line-height:1.35;margin:0 0 3px;letter-spacing:-0.01em">'+esc(moveTitleText)+'</p>'
      +'<p style="font-size:14px;color:var(--fc-text-muted);line-height:1.4;margin:0">'+esc(moveBodyText)+'</p>'
      +'</div>'
      ...
      +'<div style="flex:1;min-width:0">'
      +'<div style="'+eyebrow+';margin-bottom:8px">SAFE TO SPEND</div>'
      +'<div style="font-size:38px;font-weight:700;color:var(--fc-text);letter-spacing:-0.035em;line-height:1;font-variant-numeric:tabular-nums;margin-bottom:5px">'+fmt(safeToSpend)+'</div>'
      +'<div style="font-size:13px;color:var(--fc-text-muted)">After bills, goals &amp; buffer</div>'
      +'</div>'
      +'<div style="width:52px;height:52px;border-radius:50%;background:rgba(20,124,255,0.08);display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1.5px solid rgba(20,124,255,0.15)">'
      +'<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--fc-accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>'
      +'</div></div></div>'

      // 4. Two-column row: Upcoming Bill + Cash Flow
      +'<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:0 16px 14px">'
      +'<div style="'+card('16px')+'">'
      +'<div style="'+eyebrow+';margin-bottom:8px">UPCOMING BILL</div>'
      +nextBillHtml+'</div>'
      +'<div style="'+card('16px')+'">'
      +'<div style="'+eyebrow+';margin-bottom:8px;display:flex;align-items:center;justify-content:space-between"><span>CASH FLOW</span>'+cfArrow+'</div>'
      +'<div style="font-size:22px;font-weight:700;color:'+(cashFlow>=0?'var(--fc-success)':'var(--fc-danger)')+';letter-spacing:-0.03em;font-variant-numeric:tabular-nums;margin-bottom:6px">'+(cashFlow>=0?'+':'')+fmt(cashFlow)+'</div>'
      +'<div style="font-size:11px;color:var(--fc-text-muted);margin-bottom:2px">Income <span style="font-weight:600;color:var(--fc-text)">'+fmt(monthIncome)+'</span></div>'
      +'<div style="font-size:11px;color:var(--fc-text-muted);margin-bottom:8px">Expenses <span style="font-weight:600;color:var(--fc-text)">'+fmt(monthSpend)+'</span></div>'
      +linkBtn("FCApp.switchTab('activity')", 'View cash flow ›')
      +'</div></div>'

      // 5. Budget Progress
      +'<div style="margin:0 16px 14px;'+card()+'">'
      +'<div style="'+eyebrow+';margin-bottom:14px">BUDGET PROGRESS</div>'
      +budgetHtml+'</div>'

      // 6. Goals Progress
      +'<div style="margin:0 16px 14px;'+card()+'">'
      +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">'
      +'<div style="'+eyebrow+'">GOALS PROGRESS</div>'
      +linkBtn("FCApp._openSubScreen('goals')", 'View all goals ›')
      +'</div>'+goalsHtml+'</div>'
      ...
      +linkBtn("FCApp.switchTab('activity')", 'View all activity ›')
      +'</div>'+activityHtml+'</div>'

      // 8. Financial Health Score
      +'<div style="margin:0 16px 14px;'+card()+';cursor:pointer;-webkit-tap-highlight-color:transparent" onclick="FCApp.switchTab(\'plan\')">'
      +'<div style="display:flex;align-items:center;gap:14px;margin-bottom:12px">'
      +'<div style="width:50px;height:50px;border-radius:14px;background:rgba(20,124,255,0.08);display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1.5px solid rgba(20,124,255,0.12)">'
      +'<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--fc-accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>'
      +'</div>'
      +'<div style="flex:1;min-width:0">'
      +'<div style="'+eyebrow+';margin-bottom:5px">FINANCIAL HEALTH SCORE</div>'
      +'<div style="font-size:26px;font-weight:700;color:var(--fc-text);letter-spacing:-0.03em;line-height:1;font-variant-numeric:tabular-nums">'+healthScore+' <span style="font-size:15px;font-weight:400;color:var(--fc-text-muted)">/ 100</span></div>'
      +'</div>'
      +'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--fc-text-faint)" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg>'
      +'</div>'
      +bar(healthScore,healthColor)
      +'<div style="font-size:13px;color:var(--fc-text-muted);margin-top:10px;line-height:1.4">'+esc(healthMsg)+'</div></div>'

      // 9. Quick Actions 2x2 grid
      +'<div style="margin:0 16px 6px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px">'
      +qaBtn("FCApp.showAddTransactionSheet&&FCApp.showAddTransactionSheet()", '<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>', 'Add Transaction', 'Record income or expense', 'var(--fc-accent-soft)', 'var(--fc-accent)')
      +qaBtn("FCApp.showBillSheet&&FCApp.showBillSheet()", '<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>', 'Add Bill', 'Track and never miss', 'var(--fc-success-soft)', 'var(--fc-success)')
      +qaBtn("FCApp._openBudgetWizard&&FCApp._openBudgetWizard()", '<circle cx="12" cy="12" r="9"/><path d="M9 12h6"/><path d="M12 9v6"/>', 'Create Budget', 'Plan with confidence', 'var(--fc-electric-soft)', 'var(--fc-electric)')
      +qaBtn("FCApp.switchTab('wealth')", '<path d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4"/>', 'Net Worth', 'See your full picture', 'var(--fc-accent-soft)', 'var(--fc-accent)')
      +'</div>'

      // 10. Disclaimer
      +'<p style="font-size:10px;color:var(--fc-text-faint);text-align:center;padding:8px 24px 16px;margin:0">FlowCheck is not a bank · Not financial advice</p>';
  }
  ...
  function _renderPlanSTS() {
    const amtEl    = document.getElementById('plan-sts-amount');
    const descEl   = document.getElementById('plan-sts-desc');
    const statusEl = document.getElementById('plan-sts-status');
    if (!amtEl) return;

    const cash        = FCData.calcCash ? FCData.calcCash(state.accounts) : 0;
    const bills7      = _getBillsDueInDays(7);
    const bills7Total = bills7.reduce((s, b) => s + (b.amount || 0), 0);
    const goalCommit  = (state.goals || []).filter(g => g.monthly_amount > 0)
                          .reduce((s, g) => s + (g.monthly_amount || 0), 0);
    const buffer      = Math.max(200, Math.round(cash * 0.05));
    const sts         = Math.max(0, cash - bills7Total - goalCommit - buffer);

    const stsColor = sts < 200 ? 'var(--fc-danger)' : sts < 500 ? 'var(--fc-warning)' : 'var(--pl-sts-color)';
    amtEl.textContent  = FCData.formatCurrency(sts);
    amtEl.style.color  = stsColor;

    const items = [];
    if (bills7.length) {
      items.push({ ok: true,  label: `${bills7.length} bill${bills7.length !== 1 ? 's' : ''} covered (${FCData.formatCurrency(bills7Total)})` });
    } else {
      items.push({ ok: true,  label: 'No bills due this week' });
    }
    if (goalCommit > 0) {
      items.push({ ok: true,  label: `Goals (${FCData.formatCurrency(goalCommit)}/mo)` });
    }
    items.push({ ok: false, label: `${FCData.formatCurrency(buffer)} buffer held` });

    if (statusEl) {
      statusEl.innerHTML = items.map(i =>
        `<div class="plan-sts-item">
          <div class="plan-sts-check${i.ok ? '' : ' plan-sts-check--neutral'}">
            ${i.ok ? '<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="3.5" stroke-linecap="round" aria-hidden="true"><path d="M20 6L9 17l-5-5"/></svg>' : ''}
          </div>
  ...
  function _renderPlanMove() {
    const card  = document.getElementById('plan-move-card');
    const icon  = document.getElementById('plan-move-icon');
    const title = document.getElementById('plan-move-title');
    const sub   = document.getElementById('plan-move-sub');
    const btns  = document.getElementById('plan-move-btns');
    if (!title) return;
    ...
    const _MOVE_COLORS = {
      danger:   { bg: 'var(--pl-soft-red)',   bd: 'var(--pl-soft-red-bd)',   iBg: 'rgba(239,68,68,0.18)',  iColor: 'var(--fc-danger)', emoji: '⚠️' },
      warn:     { bg: 'var(--pl-soft-amber)', bd: 'var(--pl-soft-amber-bd)', iBg: 'rgba(245,158,11,0.18)', iColor: 'var(--fc-warning)', emoji: '💡' },
      info:     { bg: 'var(--pl-soft-blue)',  bd: 'var(--pl-soft-blue-bd)',  iBg: 'rgba(20,124,255,0.18)', iColor: 'var(--fc-accent)', emoji: '💡' },
      positive: { bg: 'var(--pl-soft-green)', bd: 'var(--pl-soft-green-bd)', iBg: 'rgba(22,163,74,0.18)',  iColor: 'var(--fc-success)', emoji: '✓'  },
    };

    if (!top) {
      const c = _MOVE_COLORS.positive;
      if (card)  { card.style.background = c.bg;   card.style.borderColor = c.bd; }
      if (icon)  { icon.style.background = c.iBg;  icon.style.color = c.iColor;   icon.textContent = c.emoji; }
      title.textContent = "You're on track";
      if (sub)  sub.textContent  = 'No urgent actions needed. Keep it up!';
      if (btns) btns.innerHTML   = '';
      return;
    }

    const c = _MOVE_COLORS[top.type] || _MOVE_COLORS.info;
    if (card)  { card.style.background = c.bg;  card.style.borderColor = c.bd; }
    if (icon)  { icon.style.background = c.iBg; icon.style.color = c.iColor; icon.textContent = c.emoji; }
    title.textContent = top.title || top.label || 'Heads up';
    if (sub)  sub.textContent  = top.body   || '';
    if (btns && top.action && top.tap) {
      const tapSrc = top.tap.toString();
      btns.innerHTML = `<button class="plan-move-btn-primary" type="button" onclick="(${tapSrc})()">${esc(top.action)}</button>`;
    } else if (btns) {
      btns.innerHTML = '';
    }
  }

  function _renderPlanForecast(periodSpend, budgetLimit, now) {
    const projEl   = document.getElementById('plan-forecast-projected');
    const spentEl  = document.getElementById('plan-forecast-spent');
    const budgetEl = document.getElementById('plan-forecast-budget');
    const daysEl   = document.getElementById('plan-forecast-days');
    const paceEl   = document.getElementById('plan-forecast-pace');
    const msgEl    = document.getElementById('plan-forecast-msg');
    if (!projEl) return;
    ...
    const isOver      = budgetLimit > 0 && projected > budgetLimit;

    projEl.textContent = FCData.formatCurrency(projected);
    projEl.style.color = isOver ? 'var(--fc-danger)' : 'var(--fc-success)';
    if (spentEl)  spentEl.textContent  = FCData.formatCurrency(periodSpend);
    if (budgetEl) budgetEl.textContent = budgetLimit > 0 ? FCData.formatCurrency(budgetLimit) : 'Not set';
    if (daysEl)   daysEl.textContent   = `${daysLeft}d`;
    if (paceEl)   paceEl.textContent   = `${FCData.formatCurrency(Math.round(dailyAvg))}/day`;

    if (msgEl) {
      if (isOver) {
        const over = projected - budgetLimit;
        msgEl.className = 'plan-forecast-msg plan-forecast-msg--warn';
        msgEl.innerHTML = `⚠️ On pace to finish ${FCData.formatCurrency(over)} over budget.`;
        msgEl.style.display = '';
      } else if (!isOver && budgetLimit > 0 && daysLeft < 8) {
        const under = budgetLimit - projected;
        msgEl.className = 'plan-forecast-msg plan-forecast-msg--ok';
        msgEl.innerHTML = `✓ On track — ${FCData.formatCurrency(under)} under budget.`;
        msgEl.style.display = '';
      } else {
  ...
  function _buildFixPlan(periodSpendTxns) {
    ...
    for (const [cat, spent] of Object.entries(catMap)) {
      ...
      if (over > 0) {
        fixes.push({
          cat, spent, limit: b.limit, over,
          emoji: (typeof FCData.categoryEmoji === 'function') ? FCData.categoryEmoji(cat) : '📦',
          color: FCData.categoryColor ? FCData.categoryColor(cat) : '#64748B',
        });
      }
    }
  ...
  function _renderPlanFix(periodSpendTxns) {
    const card   = document.getElementById('plan-fix-card');
    const rowsEl = document.getElementById('plan-fix-rows');
    if (!card || !rowsEl) return;
    const fixes = _buildFixPlan(periodSpendTxns);
    if (!fixes.length) { card.style.display = 'none'; return; }
    card.style.display = '';
    const totalOver = fixes.reduce((s, f) => s + f.over, 0);
    rowsEl.innerHTML = fixes.slice(0, 4).map(f => {
      const reduceBy = Math.ceil(f.over / 2); // suggest cutting overage in half
      return `<div class="plan-fix-row" onclick="FCApp.openCategoryBudgetSheet('${esc(f.cat)}',${f.limit})" role="button">
        <div class="plan-fix-icon" style="background:${f.color}22"><span style="font-size:14px">${f.emoji}</span></div>
        <div style="flex:1;min-width:0">
          <div class="plan-fix-name">${esc(f.cat)}</div>
          <div style="font-size:10px;color:var(--pl-secondary);margin-top:1px">Reduce by ${FCData.formatCurrency(reduceBy)} to get on track</div>
        </div>
        <span class="plan-fix-amt">+${FCData.formatCurrency(f.over)}</span>
      </div>`;
    }).join('') +
    `<div class="plan-fix-btn-row">
      <button class="plan-fix-btn-primary" onclick="FCApp.switchTab('plan')" type="button">Fix my plan</button>
      <button class="plan-fix-btn-secondary" onclick="this.closest('#plan-fix-card').style.display='none'" type="button">Dismiss</button>
    </div>`;
  }
  ...
  function _renderPlanSubs() {
    const totalEl = document.getElementById('plan-subs-total');
    const countEl = document.getElementById('plan-subs-count');
    if (!totalEl) return;
    const subs  = _detectSubscriptions();
    const total = subs.reduce((s, sub) => {
      const perMonth = sub.freq === 'wk' ? (sub.amount || 0) * 4.33
                     : sub.freq === 'yr'  ? (sub.amount || 0) / 12
                     : sub.freq === '2mo' ? (sub.amount || 0) / 2
                     : (sub.amount || 0);
      return s + perMonth;
    }, 0);
    totalEl.textContent = subs.length ? FCData.formatCurrency(Math.round(total)) : '—';
    if (countEl) countEl.textContent = subs.length
      ? `${subs.length} subscription${subs.length !== 1 ? 's' : ''}/mo`
      : 'No recurring charges found';
  }

  function _buildDailyDelta() {
    const now      = new Date();
    const cut1d    = new Date(now.getTime() -     86400000);
    const cut2d    = new Date(now.getTime() - 2 * 86400000);
    const txns     = state.transactions || [];
    const today    = txns.filter(t => _isSpendTxn(t) && FCData.parseDateLocal(t.date) >= cut1d);
    const yest     = txns.filter(t => _isSpendTxn(t) && FCData.parseDateLocal(t.date) >= cut2d && FCData.parseDateLocal(t.date) < cut1d);
    const deltas   = [];
    const todayAmt = today.reduce((s, t) => s + (t.amount || 0), 0);
    const yesterdayAmt = yest.reduce((s, t) => s + (t.amount || 0), 0);
    if (today.length) {
      deltas.push({ emoji: '💳', label: `${today.length} new txn${today.length !== 1 ? 's' : ''}`, val: FCData.formatCurrency(todayAmt), color: 'var(--fc-danger)' });
    }
    if (todayAmt > 0 && yesterdayAmt > 0) {
      const diff = todayAmt - yesterdayAmt;
      deltas.push({ emoji: diff > 0 ? '↑' : '↓', label: diff > 0 ? 'Up vs yesterday' : 'Down vs yesterday', val: FCData.formatCurrency(Math.abs(diff)), color: diff > 0 ? 'var(--fc-danger)' : 'var(--fc-success)' });
    }
    const today0 = new Date(); today0.setHours(0,0,0,0);
    const billDue = (state.bills || []).find(b => {
      if (b.status === 'paid' || !b.due_date) return false;
      const d = FCData.parseDateLocal(b.due_date); d.setHours(0,0,0,0);
      return d.getTime() === today0.getTime();
    });
    if (billDue) deltas.push({ emoji: '📅', label: `Due today: ${billDue.name}`, val: FCData.formatCurrency(billDue.amount || 0), color: 'var(--fc-warning)' });
    return deltas;
  }

  function _renderPlanDelta() {
    const listEl = document.getElementById('plan-delta-list');
    if (!listEl) return;
    const deltas = _buildDailyDelta();
    if (!deltas.length) {
      listEl.innerHTML = '<div class="plan-empty" style="padding:4px 0"><div class="plan-empty-sub">No changes today</div></div>';
      return;
    }
    listEl.innerHTML = deltas.map(d =>
      `<div class="plan-delta-item">
        <div class="plan-delta-dot" style="background:rgba(127,127,127,0.12);font-size:12px">${d.emoji}</div>
        <span class="plan-delta-lbl">${esc(d.label)}</span>
        <span class="plan-delta-val" style="color:${d.color}">${esc(d.val)}</span>
      </div>`).join('');
  }
    ...
    if (!state.goals.length) {
      ...
      state.goals.map(g => {
        ...
        if (pct >= 100) {
          ...
          statusBg    = pct >= 75 ? 'rgba(26,196,240,0.12)' : pct > 0 ? 'var(--fc-bg-elevated-2)' : 'var(--fc-bg-elevated-2)';
          statusBorder= pct >= 75 ? 'rgba(26,196,240,0.3)' : pct > 0 ? 'var(--fc-border-strong)' : 'var(--fc-border)';
          ...
          <div class="fc-goal-card" style="cursor:pointer" onclick="FCApp.editGoal('${esc(g.id)}')" role="button">
            ...
            <div style="width:64px;height:64px;position:relative;flex-shrink:0">
              <svg width="64" height="64" viewBox="0 0 64 64" style="transform:rotate(-90deg)" aria-label="${pct}%">
                <circle cx="32" cy="32" r="${r}" stroke="var(--fc-border)" stroke-width="6" fill="none"/>
  ...
  /* ─── Wealth: recommendation engine ─── */
  function _buildWealthMove() {
    const accts = state.accounts || [];
    const goals = state.goals || [];
    const cash = FCData.calcCash ? FCData.calcCash(accts) :
      accts.filter(a=>a.type==='depository').reduce((s,a)=>s+(a.balance_current||a.balance||0),0);
    const debtAccts = accts.filter(a=>a.type==='credit'||a.type==='loan');
    const cards     = debtAccts.filter(a=>a.type==='credit');
    const efGoal    = goals.find(g=>/emergency|starter/i.test(g.name||''));
    const efCurrent = efGoal ? (efGoal.current||0) : 0;
    const bills7    = typeof _getBillsDueInDays === 'function' ? _getBillsDueInDays(7).reduce((s,b)=>s+(b.amount||0),0) : 0;
    const goalCommit= goals.filter(g=>(g.monthly_amount||0)>0).reduce((s,g)=>s+(g.monthly_amount||0),0);
    const sts       = Math.max(0, cash - bills7 - goalCommit - Math.max(200, Math.round(cash*0.05)));

    if (!efGoal && cash>500) return { accent:'var(--wv-green)',
      title:'Build a Starter Emergency Fund',
      body:'A $1,000 buffer protects you from unexpected expenses — it\'s the highest-ROI financial move you can make.',
      cta:'Set Up Goal', ctaAlt:'Not now', primary:()=>{ FCApp.showAddGoalSheet && FCApp.showAddGoalSheet(); } };

    if (efGoal && efCurrent<1000) {
      const wk = Math.min(50, Math.max(10, Math.round(sts/8)));
      return { accent:'var(--wv-green)',
        title:`${FCData.formatCurrency(1000-efCurrent)} left to fund your Emergency Fund`,
        body:`Save ${FCData.formatCurrency(wk)}/week and you'll have a starter fund in about ${Math.ceil((1000-efCurrent)/(wk*4.3))} months.`,
        cta:'View Savings', ctaAlt:'Got it', primary:()=>{ FCApp.switchWealthTab('savings'); } };
    }
    ...
    if (utilPct>30 && sts>50) {
      const pay = Math.min(200, Math.round(sts*0.2));
      return { accent:'var(--wv-red)',
        title:`Credit utilization at ${utilPct}% — above the healthy 30% threshold`,
        body:`Pay an extra ${FCData.formatCurrency(pay)} toward your card balance to protect your credit score.`,
        cta:'View Debt', ctaAlt:'Dismiss', primary:()=>{ FCApp.switchWealthTab('debt'); } };
    }

    const behindGoal = goals.find(g=>{
      if ((g.pct||0)>=100||!g.target_date) return false;
      ...
      return (g.pct||0)<expPct-10;
    });
    if (behindGoal && sts>20) {
      const c=Math.min(25,Math.round(sts*0.1));
      return { accent:'var(--wv-goal)',
        title:`${esc(behindGoal.name)} is falling behind schedule`,
        body:`Add ${FCData.formatCurrency(c)} now to get back on track for your target date.`,
        cta:'View Goals', ctaAlt:'Dismiss', primary:()=>{ FCApp.switchWealthTab('goals'); } };
    }

    const histVals = Object.values(state.nwHistory||{});
    const growing  = histVals.length>=2 && histVals[histVals.length-1]>histVals[0];
    return { accent:'var(--wv-blue)',
      title: growing ? 'Net worth is growing — keep it up' : 'You\'re building wealth',
      body:'Every deposit, every payoff, every goal hit compounds over time. You\'re doing it right.',
      cta:'See Details', ctaAlt:null, primary:()=>{} };
  }
  ...
  function _renderSavings() { _renderWealthSavings(); }

  function _renderSavings_COMPAT() {
      ...
      summaryEl.innerHTML = `
        <div class="fc-card" style="margin:0 16px 4px;padding:20px;background:linear-gradient(135deg,rgba(26,196,240,0.10),rgba(37,99,235,0.08));display:flex;align-items:center;gap:16px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div class="fc-amount" style="font-size:28px;font-weight:700;letter-spacing:-0.02em;color:var(--fc-text)">${FCData.formatCurrency(total)}</div>
    ...
    const renderAcctRow = (a, isLast) => {
      ...
      return `<div style="display:flex;align-items:center;gap:13px;padding:14px 16px;${isLast ? '' : 'border-bottom:0.5px solid var(--fc-border)'}">
  ...
  function _renderDebt() { _renderWealthDebt(); }

  function _renderDebt_COMPAT() {
    const allAccts  = state.accounts || [];
      ...
      if (segments.length > 0) {
        ...
        donutSvg = `
          <svg viewBox="0 0 104 104" width="104" height="104" style="flex-shrink:0" aria-hidden="true">
            ...
            <text x="${cx}" y="${cy - 4}" text-anchor="middle" fill="var(--fc-text)" font-size="11" font-weight="800" font-family="inherit">${segments.length > 0 ? '−' : ''}${totalDebt >= 1000 ? `$${(totalDebt/1000).toFixed(0)}K` : `$${Math.round(totalDebt)}`}</text>
            <text x="${cx}" y="${cy + 9}" text-anchor="middle" fill="var(--fc-text-faint)" font-size="8" font-family="inherit">total</text>
          </svg>`;
      }
        ...
        <div class="fc-card" style="margin:0 16px 4px;padding:18px;background:linear-gradient(135deg,rgba(255,69,58,0.08),rgba(37,99,235,0.06))">
          <div style="display:flex;align-items:center;gap:16px">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div class="fc-amount" style="font-size:26px;font-weight:700;letter-spacing:-0.02em;color:var(--fc-danger)">−${FCData.formatCurrency(totalDebt)}</div>
              <!-- Legend -->
          ...
          ${totalLimit > 0 ? `
          <div style="margin-top:14px;padding-top:12px;border-top:0.5px solid var(--fc-border)">
  ...
  function _attachDataListeners() {
    ...
    FCData.listenToNetWorthHistory(history => {
      ...
      if (state.tab === 'home') _renderPremiumHomeChart(FCData.calcNetWorth(state.accounts));
      if (state.tab === 'insights') _renderInsights();
    });
  }
